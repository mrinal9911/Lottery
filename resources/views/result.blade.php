...
<!-- Main Filter Form -->
<form method="POST" id="result_form" action="{{ route('result') }}" class="default">
    @csrf
...
<div id="customModal" class="custom-modal" style="display:none;">
    <div class="custom-modal-content">
        ...
        <form id="lotteryForm" action="{{ route('lottery-results.store') }}" method="POST">
            @csrf
...
<script>
    ...
    $(function() {
        ...
        $('#lotteryForm').submit(function(e) {
            ...
            $.ajax({
                url: "{{ route('lottery-results.store') }}",
                method: "POST",
                data: formData,
                success: function(response) {
                    alert("✅ Result saved successfully!");
                    $('#customModal').hide();
                    location.reload(); // You can remove this and update table manually if needed
                },
                error: function(xhr) {
                    let err = xhr.responseJSON?.message || '❌ Error saving result.';
                    alert(err);
